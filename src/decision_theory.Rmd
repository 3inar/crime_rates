...
$$
In particular we are interested in the risk $E(L(\mbox{cpp},\hat{\mbox{cpp}}^s))=MSE(\mbox{cpp},\hat{\mbox{cpp}}^s)$. Let $f(n;\mbox{cpp})$ be the likelihood, where $n=(n_1,\ldots,n_m)$ are the (observed) number of crimes reported. According to our model we have: 
$$
\begin{split}
